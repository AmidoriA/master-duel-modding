...
- **Link over-frame**: Link frames are supported for Auto-create and Custom OF; active `CARD_Prop` markers get a synthesized silver triangular bevel + black inset + orange/red glow as a topmost overlay after compose (`Link.png` only has inactive flat triangles; inactive stay dark)
