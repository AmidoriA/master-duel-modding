# SAM 2 point cutout (Custom OF)

Floowan can extract a **specific** part of Master Duel card art with Meta
[Segment Anything 2](https://github.com/facebookresearch/sam2) via a **point
prompt** (click a pixel → mask). This sits beside rembg `isnet-anime` and does
not replace the default Auto-create path.
...
Core API: `Sam2PointCutoutService.PrepareSubjectWithPointAsync(path, x, y)` →
`(Image<Rgba32> source, Image<L8> mask)` for Custom OF Card Art layering.

## Model download

**Automatic (app):** first use of **SAM point cutout…** downloads
...
1. Click **SAM point cutout…** (hidden if `FLOOWAN_SAM2=0`).
2. Floowan extracts live card art and shows it in the preview.
3. Click the subject you want (e.g. dragon, not rider).
4. SAM 2 runs; the cutout becomes the Card Art subject (same Apply path as rembg).

## Feature flag

| Env | Effect |
|-----|--------|
| unset / anything else | SAM entry shown |
| `FLOOWAN_SAM2=0` | SAM button hidden |
...
See [THIRD_PARTY_NOTICES.md](../THIRD_PARTY_NOTICES.md) (Meta SAM 2 Apache-2.0;
